...
export function resolveWatchConfig(): WatchConfig {
  ...
  try {
    ...
    throw new Error(`无法读取 Watch Blive 配置：${file}`, { cause: error });
  }
  ...
  if (!result.success) {
    const details = result.error.issues
      ...
      .join('；');
    throw new Error(`Watch Blive 配置不合法：${details}`);
  }
...
export function watchDataDirectory() {
  return resolve(
    process.env.WATCH_BLIVE_DATA_DIR ?? join(app.isPackaged ? homedir() : process.cwd(), '.ciel'),
  );
}
