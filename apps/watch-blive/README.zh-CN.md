...
`watch-blive` 把真实的 B 站直播页嵌进 Electron `<webview>`，把直播间的声音和画面送进 [@cieljs/perception](../../packages/perception/README.zh-CN.md)，再让 Ciel Agent 一边看、一边记、一边通过 `send_danmaku` 接话。
...
| 场景                          | 数据根目录                                                       |
| ----------------------------- | ---------------------------------------------------------------- |
| 设置了 `WATCH_BLIVE_DATA_DIR` | 该目录（优先级最高）                                             |
| 打包后运行                    | `~/.ciel`                                                        |
| 开发运行                      | `<process.cwd()>/.ciel`，仓库里已有一份 `apps/watch-blive/.ciel` |
...
> [!NOTE]
> `WatchBliveRuntime` 在首次创建时就固定了选项，所以 `ai.*`、`interaction.*`、`wake` 和 `ffmpegPath` 的修改要重启应用才生效。侧栏每秒重读一次配置文件，配置错误和模型文件缺失会即时显示出来。
...
```bash
vp run watch-blive#dev     # Electron 桌面应用（仓库根 README 也记录了这条）
```
...
| 命令                                                           | 作用                                                             |
| -------------------------------------------------------------- | ---------------------------------------------------------------- |
| `vp run watch-blive#dev`                                       | `vld dev`，依赖 `build`；渲染进程热更新，主进程与 preload 重启。 |
| `vp run watch-blive#start`                                     | `vld preview`，不带开发服务器运行构建产物。                      |
| `vp run watch-blive#build`                                     | `vld build`，依赖 `typecheck`。                                  |
| `vp run watch-blive#typecheck`                                 | node 侧 `tsc` 加 web 侧 `vue-tsc`。                              |
| `vp run watch-blive#build:unpack`                              | 用 `electron-builder --dir` 产出免安装目录。                     |
| `vp run watch-blive#build:win` / `#build:mac` / `#build:linux` | 用 `electron-builder` 产出安装包。                               |
...
apps/watch-blive/
  ...
  src/
    main/
      ...
      runtime.ts                   WatchBliveRuntime：状态机、模式策略、资源所有权
...
- 渲染进程通过 oRPC 走 MessagePort 与主进程通信。preload 只转发同源的 `watch-blive:connect` 消息所带的端口；主进程只接受主窗口主 frame 或 Investigation 窗口主 frame，要求恰好一个端口，并在导航时丢弃连接。
...
vp run watch-blive#typecheck     # 本应用的 tsc（node）+ vue-tsc（web）
vp run watch-blive#build         # 构建 main、preload 与 renderer
```

`vp run watch-blive#test` 并不存在——本应用没有定义 `test` 任务。测试是与源码同目录的 `*.test.ts`（覆盖配置加载、页面执行、弹幕发送、评分策略、调度器、房间冷却、决策解析、探索选房、会话身份、提示词、退出协调、资源迁移、声纹，以及面向 IPC 的路由），由工作区的 Vitest projects 收进去。

应用的 `package.json` 没有 `scripts` 字段，所有可运行任务都在 [vite.config.ts](vite.config.ts) 里，通过 `vp run watch-blive#<task>` 调用。
