...
`createPerceptionContext` 来自 `prompts/perception.ts`，视觉分支显式复用包导出的 `DEFAULT_VISION_PROMPT`，听觉分支同时加入应用的 `HEARING_PROMPT` 和 `PERCEPTION_RULES`（见 12.5）。感知实例由宿主内部创建。
...
每轮上下文带随现场变化的数据，听觉数据同时携带对 ASR 结果的判读规则。其他行为约束仍放在系统提示词里；记忆判断规则只留在系统提示词的记忆小节。
...
- `PERCEPTION_RULES`：由 `createPerceptionContext` 的听觉分支注入，承载识别错误与凭空生成的前提、读不懂的内容不发弹幕、不下结论、不写入记忆、不编造解释，以及不假装听懂、不因一处可疑否定已印证的现场。

视觉 context 只描述画面块的结构。听觉 context 同时提供转写结构与可靠性规则，只在快照确实有听觉数据时注入。
