...
一个 Storage 拥有一个 PGlite / Drizzle 实例。Session、Memory、Vector 和 Trace 注册各自的模块，并在独立 PostgreSQL schema 中存储数据。公共 Pi 运行记录位于 `storage.events`。
...
所有借用 Storage 的服务必须先关闭。关闭 Manager 或 Trace 不关闭数据库，宿主最后关闭 Storage。初始化失败会回收 PGlite。
