import { resolveModelsPath } from './constants.ts';
...
export async function installModels(options: InstallModelsOptions = {}): Promise<string> {
  ...
  if (!model) throw new Error('Unsupported ASR model');
  await model.prepare(options);
  const ownFiles = new Set(model.files().map(file => file.target));
  for (const file of modelFiles(options)) {
    if (!ownFiles.has(file.target)) await installFile(file.url, file.target, options);
  }
  return resolveModelsPath();
}
