...
export class AudioNormalizer {
  ...
  write(chunk: ASRSegment): Float32Array {
    ...
    if (chunk.format && chunk.format !== 's16le') throw new Error('Unsupported PCM format');
    if (chunk.data.length % (channels * 2))
      throw new Error('Audio must contain aligned s16le PCM frames');
    if (!Number.isFinite(chunk.startAt.getTime())) throw new Error('Invalid audio startAt');
    ...
    for (const sample of mono) this.samples.push(sample);
