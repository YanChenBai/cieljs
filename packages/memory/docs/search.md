...
搜索接收一句自然语言或关键词，返回记忆、命中片段、检索方式和融合排名。下面假定 `store` 已打开，`scope` 为当前空间：

```ts
const hits = await store.search("周五有什么安排", {
...
可以在 `MemoryStore.open` 中传入 `tokenize: (text) => string[]`，接入自己的中文分词器。文档与查询使用同一个 tokenizer。全文使用分词后的文本，模糊检索保留未分词的规范化文本。查询以普通文本处理，不接受原始 SQL 或 tsquery 表达式。

更换 tokenizer 或运行时分词规则后，针对已有范围调用 `rebuildIndex({ scopes })`；它会重建文本分块，并为当前模型重新排入向量任务。`list` 与检索的范围语义一致，但 `list` 还支持 offset 分页，搜索只返回前 limit 条。
