...
import { MemoryEmbeddingIndex } from '../src/embedding-index.ts';
...
test('初始化和回滚都失败时保留两个错误', async () => {
  ...
  await expect(MemoryManager.open({ dataDir: 'test' })).rejects.toMatchObject({
...
test('索引刷新失败仍关闭数据库，并保留两个错误', async () => {
  const manager = await MemoryManager.open({ dataDir: 'test' });
  expect(client.close).not.toHaveBeenCalled();
  const flushError = new Error('flush');
  const closingError = new Error('closing');
  vi.spyOn(MemoryEmbeddingIndex.prototype, 'flush').mockRejectedValueOnce(flushError);
  client.close.mockRejectedValueOnce(closingError);
...
test('并发关闭等待索引刷新结束，再释放数据库', async () => {
  ...
  vi.spyOn(MemoryEmbeddingIndex.prototype, 'flush').mockReturnValueOnce(pending);
  const closing = manager.close();
  expect(manager.close()).toBe(closing);
